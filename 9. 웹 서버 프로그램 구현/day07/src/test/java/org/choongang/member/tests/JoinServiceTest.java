...
public class JoinServiceTest {
    ...
    void requiredFieldTest(){
        assertAll(()->requiredEachFieldTest("email",true,"이메일"),
            ()->requiredEachFieldTest("email",false,"이메일"),
                ...
                ()->requiredEachFieldTest("userName",false,"이름"),
                ()->requiredEachFieldTest("termsAgree",true,"약관")
        );
    }
    ...
    @Test@DisplayName("이미 가입된 메일인 경우 DuplicatedMemberException 발생")
    ...
    @Test@DisplayName("비밀번호 일치하지않는 경우 passwordMisMatchException 발생")
    void passwordMismatchTest(){
        BadRequestException thrown = assertThrows(BadRequestException.class,()->{
            RequestJoin form = getData();
            form.setConfirmPassword(form.getPassword().toUpperCase());
            service.process(form);
        });
        String message = thrown.getMessage();
        assertTrue(message.contains("비밀번호가 일치하지"));
    }
    @Test@DisplayName("이메일이 형식에 맞지않으면 BadRequestException 발생")
    void emailPatternTest(){
        BadRequestException thrown = assertThrows(BadRequestException.class,()->{
           RequestJoin form = getData();
           form.setEmail("stupid Email");
           service.process(form);
        });
        String message = thrown.getMessage();
        assertTrue(message.contains("이메일 형식이"));
    }
    @Test@DisplayName("비밀번호 자리수가 8자리 미만이면 BadRequestException 발생")
