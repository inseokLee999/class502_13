...
//        JPAQueryFactory factory = new JPAQueryFactory(em);
    ...
    void test6(){
        QBoardData boardData = QBoardData.boardData;
        JPAQuery<Long> query = jpaQueryFactory.select(boardData.seq.sum()).from(boardData);
    ...
    void test7(){
        ...
        JPAQuery<BoardData> query = jpaQueryFactory.selectFrom(boardData)//전체 데이터 조회할 때는 영속하다
                .leftJoin(boardData.member)
                .fetchJoin()
                .where(andBuilder)
                ...
                .orderBy(
                        new OrderSpecifier(Order.DESC,pathBuilder.get("createdAt")),//createdAt 내림차순 정렬
                        new OrderSpecifier(Order.ASC,pathBuilder.get("userName"))//username 오름차순 정렬
                );
//                .where(boardData.seq.in(2L,3L,4L));//반환 값은 booleanExpression - predicate
        List<BoardData> items = query.fetch();
        items.forEach(System.out::println);
    }
}
